...
var strings_en = {
    ...
    "Table of Contents":"Table of Contents",
    "restart presentation":"restart presentation",
    "restart?":"Restart",
    "print":"Print",
    "back":"Back"
};
...
function copyBackground(slide)
{
    processBackgrounds(slide, function(slide, background, matching)
        {
            if (matching)
             {
                var temp = background.cloneNode(true);
                slide.insertBefore(temp, slide.firstChild);
                temp.style.display = "block";
                temp.style.visibility = "visible";
                temp.className = temp.className + " printTemp";
...
function processBackgrounds(slide, processBackground)
{
    var slideClass = getClassList(slide);
      ...
      for (var i = 0; i < backgrounds.length; i++)
      {
         ...
         processBackground(slide, background, matchingBackground(slideClass, bgClass));
      }
}
...
//    start.innerHTML = "print!".localize();
      ...
      print.setAttribute("href", "javascript:printSlides()");
      print.setAttribute("title", "print all slides".localize());
      print.innerHTML = "print".localize();
      left.appendChild(print);

      var gap4 = document.createTextNode(" | ");
      left.appendChild(gap4);

      var back = document.createElement("a");
      back.setAttribute("href", "javascript:goBack()");
      back.setAttribute("title", "back".localize());
      back.innerHTML = "back".localize();
